...
// examples/cli-app/.storm/commands/vars/list/handle.ts
...
function getBuildInfo() {
  return {
    ...
    version: "0.0.1",
    buildId: "v8uin35NR5u1GSFHJVGz3LFr",
    timestamp: 1748121632361 ? Number(1748121632361) : 0,
    releaseId: "x_foRJUqh4oiGRUOA6viifnb",
    mode: "production",
    platform: "node",
    isTest,
...
var __\u03A9VarsListPayload = ["VarsListPayload", "PMw!y"];
async function handler(payload) {
  ...
  console.log(`${Object.keys(vars).map(__assignType3((key) => `${colors.bold(key)}: ${vars[key]}`, ["key", "", 'P"2!"/"'])).join("\n")}`);
}
__name(handler, "handler");
handler.__type = [() => __\u03A9VarsListPayload, () => StormPayload, "payload", "handler", "Lists all configuration parameters in the variables store.", 'PPn!7"2#"/$?%'];
