...
// examples/cli-app/.storm/commands/completions/bash/handle.ts
...
var __\u03A9CompletionsBashPayload = [true, "script", "The path to write the completion script to.", true, "config", "The configuration file to append the completion script to.", "CompletionsBashPayload", `PP&.!J4"8?#P&.$J4%8?&Mw'y`];
...
`);
  ...
  if (payload.data.config) {
    let configFile = payload.data.config === true ? "~/.bashrc" : payload.data.config;
    if (configFile.startsWith("~")) {
      configFile = join2(homedir2, configFile.replace("~", ""));
    }
    let configFileContent = "";
    if (existsSync2(configFile)) {
      configFileContent = await readFile2(configFile, "utf8");
    }
    await writeFile(configFile, configFileContent + "\n\n" + stripAnsi(script), "utf8");
    console.log(colors.dim(` > Bash completion script appended to ${configFile} configuration`));
  } else if (payload.data.script) {
    const scriptFile = typeof payload.data.script === "string" ? payload.data.script : "examples-cli-completions.sh";
...
handler.__type = [() => __\u03A9CompletionsBashPayload, () => StormPayload, "payload", "handler", "Generates a Bash shell completion script for the Examples CLI App CLI application.", 'PPn!7"2#"/$?%'];
var handle_default = handler;
export {
  __\u03A9CompletionsBashPayload,
