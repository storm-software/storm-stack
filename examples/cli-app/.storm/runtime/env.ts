...
export const mode = String($storm.vars.MODE);
...
export const build = {
  ...
  mode,
  platform: $storm.vars.PLATFORM as StormBuildInfo["platform"],
  isProduction,
  isStaging,
  isDevelopment
} as StormBuildInfo;
...
export const stormVariablesNamingStrategy = new (class extends NamingStrategy {
  ...
  public override getPropertyName(
    ...
    return name
      .replace(
        /^(VITE_|ONE_|STORM_PUBLIC_|STORM_|STORM_STACK_PUBLIC_|STORM_STACK_|NEXT_PUBLIC_|EXAMPLES_CLI_)/g,
        ""
      )
      .toUpperCase();
  }
...
if (existsSync(join(paths.config, "vars.json"))) {
  varsFile = JSON.parse(
    (await readFile(join(paths.config, "vars.json"), "utf8")) || "{}"
  );
}
if (
  existsSync(
    join(paths.config.replace(new RegExp(`/${name}$`), ""), "vars.json")
  )
) {
...
export const vars = new Proxy<StormVariables>(
  ...
  ),
  {
    get(target, prop, receiver) {
      return (
        target[prop as keyof StormVariables] ??
        varsFile[prop as keyof StormVariables]
      );
    }
  }
);
