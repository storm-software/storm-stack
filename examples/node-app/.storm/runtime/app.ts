...
>(params: StormRuntimeParams) {
  ...
  const build = () => {
    if (
      !builderConfig.handler ||
      !builderConfig.deserializer ||
      !builderConfig.serializer
    ) {
      const missing = [
    ...
    return async function appWrapper(payload: TPayload): Promise<TResult> {
      async function contextWrapper(
        ...
        } as StormContext<StormEnv, { paths: EnvPaths }, TRequest>;
              ...
              if (builderConfig.validator) {
                ...
                if (issues) {
                  return StormResponse.create(
                    getErrorFromUnknown(null, "validation", issues)
                  );
                }
              }
