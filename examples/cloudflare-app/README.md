...
# Storm Stack - Cloudflare Adapter

A package containing the Cloudflare adapter for Storm Stack builds.
...
- [Storm Stack - Cloudflare Adapter](#storm-stack---cloudflare-adapter)
...
```bash
pnpm add -D @storm-stack/cloudflare
```
...
```bash
npm install -D @storm-stack/cloudflare
```
...
```bash
yarn add -D @storm-stack/cloudflare
```
...
Run `nx test cloudflare` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint cloudflare` to run [ESLint](https://eslint.org/) on the
package.
