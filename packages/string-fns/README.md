...
```bash
pnpm add -D @storm-stack/string-fns
```
...
```bash
npm install -D @storm-stack/string-fns
```
...
```bash
yarn add -D @storm-stack/string-fns
```
...
Run `nx build string-fns` to build the library.

### Running unit tests

Run `nx test string-fns` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint string-fns` to run [ESLint](https://eslint.org/) on the package.
