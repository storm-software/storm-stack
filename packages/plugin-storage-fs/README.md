...
# Storm Stack - File System Storage Adapter

A package containing a Storm Stack storage adapter to write data to the file system.
...
- [Storm Stack - File System Storage Adapter](#storm-stack---file-system-storage-adapter)
...
```bash
pnpm add @storm-stack/plugin-log-storage
```
...
```bash
npm install @storm-stack/plugin-log-storage
```
...
```bash
yarn add @storm-stack/plugin-log-storage
```
...
Run `nx build plugin-log-storage` to build the library.

### Running unit tests

Run `nx test plugin-log-storage` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-log-storage` to run [ESLint](https://eslint.org/) on the
package.
