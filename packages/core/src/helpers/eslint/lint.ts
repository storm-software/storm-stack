...
import { installPackages } from "../install-package";
...
> {
  try {
    ...
    let results = await eslint.lintFiles(
      context.resolvedTsconfig.fileNames.filter(
        fileName =>
...
export async function runLintCheck<TOptions extends Options = Options>(
  log: LogFn,
  context: Context<TOptions>,
  opts: {
    lintDuringBuild?: boolean;
  ...
  if (config.exists) {
    ...
    return lint(log, context, eslintFile, {
      lintDuringBuild,
      eslintOptions,
      reportErrorsOnly,
      maxWarnings,
