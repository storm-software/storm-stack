...
import { reflectType } from "../deepkit";
import { getDotenvDefaultTypeDefinition } from "./resolve-dotenv";
...
export async function reflectDotenvTypes<TOptions extends Options = Options>(
  ...
  if (!context.dotenv?.types) {
    ...
    params = isObject(context.dotenv.types)
  ...
  if (params.variables) {
    ...
    if (
      ...
      existsSync(
        joinPaths(context.projectRoot, result.variables.typeDefinition.file)
      )
    ) {
  ...
  if (params.secrets) {
    ...
    if (
      existsSync(
        joinPaths(context.projectRoot, result.secrets.typeDefinition.file)
      )
    ) {
