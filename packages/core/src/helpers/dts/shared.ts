...
export function generateConfig(reflection: ReflectionClass<any>) {
  if (!reflection) {
    return "{}";
  }

  return `{
  ${generateConfigInner(reflection)}

  [key: string]: any;
}`;
}

export function generateConfigInner(reflection: ReflectionClass<any>) {
  if (!reflection) {
    return "";
  }
...
declare global {
  type StormConfig = ${generateConfig(reflection)};
