 ...
 */
export async function prepareGenerate(context: Context, hooks: EngineHooks) {
  context.log(
    LogLevelLabel.TRACE,
    `Generating the pre-build artifacts for the Storm Stack project.`
  );

  await hooks.callHook("prepare:generate", context).catch((error: Error) => {
    context.log(
      LogLevelLabel.ERROR,
      `An error occurred while generating the pre-build artifacts for the Storm Stack project: ${error.message} \n${error.stack ?? ""}`
    );

    throw new Error(
      "An error occurred while generating the pre-build artifacts for the Storm Stack project",
      { cause: error }
    );
  });
}
