...
A package containing a Storm Stack log adapter to write log messages to
...
```bash
pnpm add @storm-stack/plugin-log-sentry
```
...
```bash
npm install @storm-stack/plugin-log-sentry
```
...
```bash
yarn add @storm-stack/plugin-log-sentry
```
...
Run `nx build plugin-log-sentry` to build the library.

### Running unit tests

Run `nx test plugin-log-sentry` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-log-sentry` to run [ESLint](https://eslint.org/) on the
package.
