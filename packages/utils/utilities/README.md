...
This package contains various utils-utilities functions that serve a wide range
of purposes. These utils-utilities are used by other packages in the storm-stack
monorepo and any other projects that could benefit from their functionality.
...
```bash
pnpm add -D @storm-stack/utils-utilities
```
...
```bash
npm install -D @storm-stack/utils-utilities
```
...
```bash
yarn add -D @storm-stack/utils-utilities
```
...
Run `nx build utils-utilities` to build the library.

### Running unit tests

Run `nx test utils-utilities` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint utils-utilities` to run [ESLint](https://eslint.org/) on the
package.
