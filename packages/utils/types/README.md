...
A package containing various type checking functions and utility utils-types
...
```bash
pnpm add -D @storm-stack/utils-types
```
...
```bash
npm install -D @storm-stack/utils-types
```
...
```bash
yarn add -D @storm-stack/utils-types
```
...
Run `nx test utils-types` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint utils-types` to run [ESLint](https://eslint.org/) on the package.
