...
```bash
pnpm add -D @storm-stack/utils-string-fns
```
...
```bash
npm install -D @storm-stack/utils-string-fns
```
...
```bash
yarn add -D @storm-stack/utils-string-fns
```
...
Run `nx build utils-string-fns` to build the library.

### Running unit tests

Run `nx test utils-string-fns` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint utils-string-fns` to run [ESLint](https://eslint.org/) on the
package.
