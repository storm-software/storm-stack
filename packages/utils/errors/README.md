...
This package provides a wrapper for the JavaScript `Error` object and a simple
way to create custom utils-errors through error code strings that can be used
...
```bash
pnpm add -D @storm-stack/utils-errors
```
...
```bash
npm install -D @storm-stack/utils-errors
```
...
```bash
yarn add -D @storm-stack/utils-errors
```
