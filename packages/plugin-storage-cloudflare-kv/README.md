...
# Storm Stack - Cloudflare KV Storage Plugin

A package containing a Storm Stack plugin for Cloudflare KV storage.
...
- [Storm Stack - Cloudflare KV Storage Plugin](#storm-stack---cloudflare-kv-storage-plugin)
...
```bash
pnpm add -D @storm-stack/plugin-log-stream
```
...
```bash
npm install -D @storm-stack/plugin-log-stream
```
...
```bash
yarn add -D @storm-stack/plugin-log-stream
```
...
Run `nx build plugin-log-stream` to build the library.

### Running unit tests

Run `nx test plugin-log-stream` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-log-stream` to run [ESLint](https://eslint.org/) on the
package.
