...
export function loadConfigFromContext(
  ...
  return defu(
    ...
    isSetObject(context.reflections?.config?.types?.params)
      ? context.reflections.config.types.params
          ?.getProperties()
          .reduce((ret, prop) => {
            ret[prop.name] = parsed[prop.name];
            return ret;
          }, {})
      : {}
  );
...
export async function loadEnv<
  TEnv extends DotenvParseOutput = DotenvParseOutput
>(context: ConfigPluginContext, options: ConfigPluginOptions): Promise<TEnv> {
  ...
  return defu(
    loadConfigFromContext(context, process.env),
    project,
    workspace,
    config
  ) as TEnv;
