...
# Storm Stack - Configuration Plugin

A package containing a Storm Stack plugin for injecting configuration parameter values into the code so that they're accessible at runtime.
...
- [Storm Stack - Configuration Plugin](#storm-stack---configuration-plugin)
...
```bash
pnpm add @storm-stack/plugin-config
```
...
```bash
npm install @storm-stack/plugin-config
```
...
```bash
yarn add @storm-stack/plugin-config
```
...
Run `nx build plugin-config` to build the library.

### Running unit tests

Run `nx test plugin-config` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-config` to run
