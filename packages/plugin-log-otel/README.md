...
A package containing a Storm Stack log adapter to write logs to
[an OpenTelemetry collector](https://opentelemetry.io/).
...
```bash
pnpm add @storm-stack/plugin-log-otel
```
...
```bash
npm install @storm-stack/plugin-log-otel
```
...
```bash
yarn add @storm-stack/plugin-log-otel
```
...
Run `nx build plugin-log-otel` to build the library.

### Running unit tests

Run `nx test plugin-log-otel` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-log-otel` to run [ESLint](https://eslint.org/) on the
package.
