...
This package contains various [Nx](https://nx.dev) executors and generators that help in managing Storm workspaces. It also contains a few utilities that can be used by other executors and generators.
...
```bash
npm install -D @storm-software/nx
```
...
Run a build on the TypeScript project using base tsup executor
...
```cmd 
nx run my-project:ts-build
```

**Please note:** _The ts-build executor should be included in the desired projects's `project.json` file._ 
...
Run the build process for a TypeScript server-side package
...
```cmd 
nx run my-project:ts-server-build
```

**Please note:** _The ts-server-build executor should be included in the desired projects's `project.json` file.All required options must be included in the `options` property of the json._ 
...
| Option    | Type   | Description   | Default   | 
| --------- | ------ | ------------- | --------- | 
| **platform \***    | "browser" \| "neutral" \| "node" \| "worker"     | Platform target for outputs.     | "node"     | 
...
Generate the schema used to describe the Storm configuration
...
| Option    | Type   | Description   | Default   | 
| --------- | ------ | ------------- | --------- | 
| **outputFile \***    | `string`    | The output file for the workspace configuration     | "{workspaceRoot}/packages/config/schema.json"     | 
