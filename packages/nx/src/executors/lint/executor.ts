...
export async function executorFn(
  options: StormStackLintExecutorSchema,
  ...
  writeTrace("Linting Storm Stack project");
  await engine.lint(options.autoPrepare !== false, options.autoClean !== false);

  writeTrace("Finalizing Storm Stack processes");
  await engine.finalize();

  writeTrace("Storm Stack - Lint completed");
...
export default withRunExecutor<StormStackLintExecutorSchema>(
  "Storm Stack - Lint executor",
  executorFn,
  {
    skipReadingConfig: false,
    hooks: {
      applyDefaultOptions: (options: StormStackLintExecutorSchema) => {
