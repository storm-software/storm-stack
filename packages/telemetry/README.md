...
- [Storm-Stack Telemetry Library](#storm-stack-telemetry-library)
...
A Storm-Stack library used to drive server logging and handling telemetry
...
```bash
pnpm add -D @storm-stack/telemetry
```
...
```bash
npm install -D @storm-stack/telemetry
```
...
```bash
yarn add -D @storm-stack/telemetry
```
...
Run `nx lint telemetry` to run [ESLint](https://eslint.org/) on the package.
