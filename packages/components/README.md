...
```bash
pnpm add -D @storm-stack/components
```
...
```bash
npm install -D @storm-stack/components
```
...
```bash
yarn add -D @storm-stack/components
```
...
Run `nx lint components` to run [ESLint](https://eslint.org/) on the package.
