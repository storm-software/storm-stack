...
A package containing a Storm Stack plugin that injects a consistent interface into the application for working with dates across some popular JavaScript libraries, such as `date-fns`, `dayjs`, `luxon`, and `moment.js`. Never feel the pressure to choose a date library again!
...
- [Storm Stack - Date Plugin](#storm-stack---date-plugin)
...
```bash
pnpm add @storm-stack/plugin-date
```
...
```bash
npm install @storm-stack/plugin-date
```
...
```bash
yarn add @storm-stack/plugin-date
```
