...
# Storm-Stack Telemetry Library

A Storm-Stack library used to drive server logging and handling base-telemetry
processing in a NodeJS environment
...
```bash
npm install -D @storm-stack/base-telemetry
```
...
```bash
yarn add -D @storm-stack/base-telemetry
```
...
Run `nx build base-telemetry` to build the library.

### Running unit tests

Run `nx test base-telemetry` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint base-telemetry` to run [ESLint](https://eslint.org/) on the
package.
