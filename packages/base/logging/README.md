...
# Storm Logging Package

📓 This package provides a base-logging service for the Storm stack. Included is
base logger functionality and a console logger utility used by the Storm CLI
tools and for local development.
...
```bash
pnpm add -D @storm-stack/base-logging
```
...
```bash
npm install -D @storm-stack/base-logging
```
...
```bash
yarn add -D @storm-stack/base-logging
```
...
Run `nx build base-logging` to build the library.

### Running unit tests

Run `nx test base-logging` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint base-logging` to run [ESLint](https://eslint.org/) on the package.
