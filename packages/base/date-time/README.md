...
This package provides a simple to use DateTime object. It includes various
utility functions for working with dates and times, as well as a number of
formatting options. It is designed to be used in a browser or NodeJS
environment.
...
```bash
pnpm add -D @storm-stack/base-date-time
```
...
```bash
npm install -D @storm-stack/base-date-time
```
...
```bash
yarn add -D @storm-stack/base-date-time
```
...
Run `nx build base-date-time` to build the library.

### Running unit tests

Run `nx test base-date-time` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint base-date-time` to run [ESLint](https://eslint.org/) on the
package.
