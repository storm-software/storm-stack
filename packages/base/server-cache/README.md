...
```bash
pnpm add -D @storm-stack/base-server-cache
```
...
```bash
npm install -D @storm-stack/base-server-cache
```
...
```bash
yarn add -D @storm-stack/base-server-cache
```
...
Run `nx build base-server-cache` to build the library.

### Running unit tests

Run `nx test base-server-cache` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint base-server-cache` to run [ESLint](https://eslint.org/) on the
package.
