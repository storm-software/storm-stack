...
export function withContext<TInput = any, TOutput = any>(
  ...
  return async function wrapper(input: TInput) {
    ...
    const context = {
      ...
      runtime,
      paths,
      log: log.with({ name: appName, version: appVersion, payloadId: payload.id }),
      storage,
