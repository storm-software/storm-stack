...
export function createContext(options: Options = {}, rootDir = process.cwd()) {
  ...
  eslintrc.filepath = eslintrc.filepath || "./.eslintrc-storm.json";
  ...
  biomelintrc.filepath = biomelintrc.filepath || "./.biomelintrc-storm.json";
...
${dts}`.trim()}\n`;
  ...
  const importsPromise = flattenImports(options.imports).then(imports => {
    if (imports.length === 0 && resolvers.length === 0 && !dirs?.length) {
      ...
      console.warn(
        "[storm] plugin installed but no imports has defined, see https://github.com/antfu/unplugin-auto-import#configurations for configurations"
      );
    }
    ...
    toArray(map).map(async param => {
      ...
      if (typeof definition === "string") {
        if (!presets[definition]) {
          throw new Error(`[storm] preset ${definition} not found`);
        }
