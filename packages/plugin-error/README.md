...
# Storm Stack - Error Codes Plugin

A package containing a Storm Stack base plugin for managing error messages through an external JSON file.
...
- [Storm Stack - Error Codes Plugin](#storm-stack---error-codes-plugin)
...
```bash
pnpm add @storm-stack/plugin-error
```
...
```bash
npm install @storm-stack/plugin-error
```
...
```bash
yarn add @storm-stack/plugin-error
```
...
Run `nx build plugin-error` to build the library.

### Running unit tests

Run `nx test plugin-error` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-error` to run
