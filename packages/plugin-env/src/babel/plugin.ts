 ...
 */
export default declareBabel<BabelPluginOptions, EnvPluginContext>(
  "env",
  ({ log, context }) => {
    function extractEnv(
      node: t.Identifier,
      ...
      if (node.name) {
        ...
        if (
          ...
          const envProperty = context.reflections.env.types.env.hasProperty(
            name
          )
