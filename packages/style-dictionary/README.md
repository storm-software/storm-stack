...
A package including various extensions to the Style Dictionary design token
generation utility
...
```bash
pnpm add -D @storm-stack/style-dictionary
```
...
```bash
npm install -D @storm-stack/style-dictionary
```
...
```bash
yarn add -D @storm-stack/style-dictionary
```
...
Run `nx test style-dictionary` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint style-dictionary` to run [ESLint](https://eslint.org/) on the
package.
