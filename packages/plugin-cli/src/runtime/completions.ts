...
import { colors, stripAnsi } from "${relativePath(
    joinPaths(context.artifactsPath, "commands", "completions", "bash"),
...
export function writeCompletionsZsh(
  ...
  return `${getFileHeader()}
import { colors, stripAnsi } from "${relativePath(
    joinPaths(context.artifactsPath, "commands", "completions", "zsh"),
    joinPaths(context.runtimePath, "cli"),
    false
  )}";
import { StormPayload } from "${relativePath(
