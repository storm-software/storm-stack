...
[![Version](https://img.shields.io/badge/version-1.6.2-1fb2a6.svg?style=for-the-badge&color=1fb2a6)](https://prettier.io/)&nbsp;
...
The base UI component library used by the Storm Stack frontend packages
...
```bash
pnpm add -D @storm-stack/frontend-components
```
...
```bash
npm install -D @storm-stack/frontend-components
```
...
```bash
yarn add -D @storm-stack/frontend-components
```
...
Run `nx build frontend-components` to build the library.
...
Run `nx lint frontend-components` to run [ESLint](https://eslint.org/) on the package.
