...
```bash
pnpm add -D @storm-stack/frontend-table
```
...
```bash
npm install -D @storm-stack/frontend-table
```
...
```bash
yarn add -D @storm-stack/frontend-table
```
...
Run `nx build frontend-table` to build the library.

### Running unit tests

Run `nx test frontend-table` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint frontend-table` to run [ESLint](https://eslint.org/) on the package.
